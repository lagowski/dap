"""Curated DAP knowledge for the config assistant (#689 slice 2).

Prompt-stuffed into the assistant's system prompt so its config advice is
grounded in how DAP actually works rather than hallucinated. Kept concise on
purpose — this is a *grounding* summary, not the full docs. Update it when the
configuration surface changes (runtimes, roles, gate semantics, templates).
"""
