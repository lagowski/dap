"""Tests for the per-role output parser.

The parser turns raw adapter text (LLM completion or shell stdout) into a
state diff for known roles. Roles without a declared schema are skipped
(the engine falls back to permissive structured-field merging).
"""
