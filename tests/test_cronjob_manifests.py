...
@pytest.fixture(params=MANIFEST_FILES)
def manifest_with_name(request):
...
class TestCommonProperties:
    ...
    def test_api_version(self, filename):
        manifest = _load_manifest(filename)
        assert manifest["apiVersion"] == "batch/v1"

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_kind_is_cronjob(self, filename):
        manifest = _load_manifest(filename)
        assert manifest["kind"] == "CronJob"

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_namespace_is_trading(self, filename):
        manifest = _load_manifest(filename)
        assert manifest["metadata"]["namespace"] == "trading"

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_concurrency_policy_forbid(self, filename):
        manifest = _load_manifest(filename)
        assert manifest["spec"]["concurrencyPolicy"] == "Forbid"

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_restart_policy_on_failure(self, filename):
        manifest = _load_manifest(filename)
        pod_spec = _get_pod_spec(manifest)
        assert pod_spec["restartPolicy"] == "OnFailure"

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_image_references_cfd_docker_image(self, filename):
        manifest = _load_manifest(filename)
        container = _get_container(manifest)
        assert container["image"] == "ghcr.io/dixter999/cfd-research:latest"

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_image_pull_secrets_ghcr_pull_secret(self, filename):
        manifest = _load_manifest(filename)
        pod_spec = _get_pod_spec(manifest)
        assert pod_spec["imagePullSecrets"] == [{"name": "ghcr-pull-secret"}]

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_env_from_cfd_research_env_secret(self, filename):
        manifest = _load_manifest(filename)
        container = _get_container(manifest)
        assert {"secretRef": {"name": "cfd-research-env"}} in container["envFrom"]

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_no_inline_env_vars(self, filename):
        manifest = _load_manifest(filename)
        container = _get_container(manifest)
        assert container.get("env") is None

    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_resource_limits_memory_1gi_cpu_500m(self, filename):
        manifest = _load_manifest(filename)
        container = _get_container(manifest)
    ...
    @pytest.mark.parametrize("filename", MANIFEST_FILES)
    def test_resource_requests(self, filename):
        manifest = _load_manifest(filename)
        container = _get_container(manifest)
...
class TestWeeklySweep:
    def test_weekly_sweep_schedule_saturday_0200_utc(self):
        manifest = _load_manifest("cfd-weekly-sweep.yaml")
        assert manifest["spec"]["schedule"] == "0 2 * * 6"

    def test_weekly_sweep_args_cfd_sweep_all_cfd(self):
        manifest = _load_manifest("cfd-weekly-sweep.yaml")
        container = _get_container(manifest)
...
class TestDailyWatchlist:
    def test_daily_watchlist_schedule_weekdays_0600_utc(self):
        manifest = _load_manifest("cfd-daily-watchlist.yaml")
        assert manifest["spec"]["schedule"] == "0 6 * * 1-5"

    def test_daily_watchlist_args_cfd_sweep_daily(self):
        manifest = _load_manifest("cfd-daily-watchlist.yaml")
        container = _get_container(manifest)
...
class TestEarningsSweep:
    def test_earnings_sweep_schedule_sunday_2000_utc(self):
        manifest = _load_manifest("cfd-earnings-sweep.yaml")
        assert manifest["spec"]["schedule"] == "0 20 * * 0"

    def test_earnings_sweep_args_cfd_sweep_earnings_week(self):
        manifest = _load_manifest("cfd-earnings-sweep.yaml")
        container = _get_container(manifest)
